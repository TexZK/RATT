...
Finally, the \emph{HID Report} descriptor describes the syntax and semantics
of the actual data being transferred by the device application. Because of
the custom semantics, the descriptor simply indicates 16 bytes for both
...
Instead, the \emph{in} page contains the description of the two motion
sources. They are both interpreted as jogwheels, with full 16-bit unsigned
...
For demonstration purposes, only the optical motion sensor counter will be
processed as jogwheel position by the software, while the quadrature counter
will be ignored.
...
<mapper device="RATT" author="TexZK"
 description="RATT" version="704" date="02/08/2012">
  <!-- Use the sensor as a jogwheel -->
  <map value="SENSOR" action="jogwheel"/>
  <!-- Ignore the quadrature input -->
  <map value="QUADRATURE" action="nothing"/>
</mapper>
\end{lstlisting}
\end{mdframed}
