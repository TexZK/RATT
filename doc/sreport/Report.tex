...
The most basic type of inputs come from \emph{discrete-state} input devices,
such as buttons, switches, toggles, and so on. In order to handle them, a
\emph{button matrix} often suffices, such as the one seen in \textbf{Figure~\CITEME}.
Such matrix can be scanned row-by-row through row (de)multiplexing, and then
processed by the \emph{digital inputs task} of the firmware \CITEME.
...
charlieplexing. These lights can be either on or off.
