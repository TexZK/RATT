...
60~Hz for the best feeling and user reaction time.
...
A driver will handle such interrupts, and it will give access to counters
atomically.
...
A debug module could be added for debug purposes only. By the way, an advanced
hardware debugger is almost always present in MCUs today, so there is no real
need for a debug console for a consumer DJ controller.

If the software debug module is added, it needs to be interfaced to the UART
through a driver provided by the RTOS. It also needs a task with background
priority in the case of a simple debug console -- if necessary, immediate
commands can always be issued through the hardware debugger.
